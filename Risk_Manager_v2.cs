    ...
    public class RiskManagerPlugin : Plugin
    {
        ...
        public override IList<SettingItem> Settings
        {
            get
            {
                ...
                result.Add(new SettingItemString("SessionLockTime", _flattenLockTime.HasValue ? _flattenLockTime.Value.ToString(@"hh\:mm") : "")
                    ...
                    switch (item.Name)
                    {
                        ...
                        case "SessionLockTime":
                            ...
                            if (string.IsNullOrWhiteSpace(lockRaw))
                            {
                                ...
                                _flattenLockTimer?.Dispose();
                                _flattenLockTimer = null;
                                Core.Instance.Loggers.Log("[RiskParameters] Session Lock Time disabled.");
                            }
                            else if (TimeSpan.TryParse(lockRaw, out var parsedLock))
                            {
                                _flattenLockTime = parsedLock;
                                ScheduleFlattenLockTimer();
                            }
                            break;
