...
namespace RiskParameters
{
    public class RiskManagerPlugin : Plugin
    {
        ...
        // Hardcoded to 5:00 PM Eastern — CME futures market close. Not a user setting.
        ...
        /// <summary>
        /// Returns the UTC start of the current session based on 5:00 PM Eastern time.
        ...
        public override IList<SettingItem> Settings
        {
            get
            {
                ...
                { Text = "Include Unrealized P&L in Limits", SortIndex = 6 });
                result.Add(new SettingItemDouble("FeePerMicro", _pendingFeePerMicro)
                { Text = "Fee Per Micro Contract (Round Trip $)", SortIndex = 7, Increment = 0.01, DecimalPlaces = 2 });
                result.Add(new SettingItemDouble("FeePerMini", _pendingFeePerMini)
                { Text = "Fee Per Mini Contract (Round Trip $)", SortIndex = 8, Increment = 0.01, DecimalPlaces = 2 });
                result.Add(new SettingItemString("MicroSymbols", _pendingMicroSymbols)
                { Text = "Micro Symbols (comma-separated)", SortIndex = 9 });
                result.Add(new SettingItemString("MiniSymbols", _pendingMiniSymbols)
                { Text = "Mini Symbols (comma-separated)", SortIndex = 10 });
                result.Add(new SettingItemAction("SaveSettings", new SettingItemActionDelegate(ApplyPendingSettings), 11)
                { Text = "Save Settings" });
                return result;
            }
            set
    ...
    public class RiskManagerRenderer : Renderer
    {
        ...
        protected virtual void Draw(Graphics gr)
        {
            ...
            // Reset Time (hardcoded 5:00 PM Eastern)
            gr.DrawString("Reset Time", labelFont, grayBrush, x, y);
            gr.DrawString("5:00 PM ET", valueFont, whiteBrush, col2, y);
            y += lineH;
