...
namespace RiskParameters
{
    public class RiskManagerPlugin : Plugin
    {
        ...
        /// <summary>
        /// Open time of the earliest-opened, still-open position for the monitored account.
        /// If multiple positions are open simultaneously (e.g. two symbols), the clock is
        /// anchored to whichever one was opened first. Once that position fully closes,
        /// this naturally rolls to the next-earliest still-open position (or null if flat).
        /// </summary>
        public DateTime? EarliestOpenPositionTime
        {
            get
            {
                ...
                return positions.Min(p => p.OpenTime);
            }
        }

        /// <summary>
        /// Elapsed time since the earliest open position for the monitored account was opened.
        /// Null when flat (no open positions).
        /// </summary>
        public TimeSpan? TradeClockElapsed
        {
            get
            {
                var opened = EarliestOpenPositionTime;
                if (!opened.HasValue) return null;
                var elapsed = DateTime.Now - opened.Value;
