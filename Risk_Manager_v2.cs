...
namespace RiskParameters
{
    public class RiskManagerPlugin : Plugin
    {
        ...
        private bool _settingsLockRestoredFromDisk = false;
        private static readonly string _lockStateDir = Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "RiskParametersMod");
        ...
        private Timer _tradeClockTimer;
        private DateTime? _tradeClockStartLocal;
        ...
        private void OnPositionAdded(Position pos)
        {
            ...
            // Start the trade clock immediately when the first position opens.
            // Use the position's own OpenTime so the clock is accurate even if
            // this event fires slightly after the fill (e.g. during a fast market).
            if (_tradeClockStartLocal == null)
            {
                var openTime = pos.OpenTime;
                _tradeClockStartLocal = (openTime != default && openTime.Kind != DateTimeKind.Unspecified)
                    ? openTime.ToLocalTime()
                    : DateTime.Now;
            }
        ...
        public override void Populate(PluginParameters args = null)
        {
            ...
            if (!_settingsLockRestoredFromDisk)
            {
                _settingsLockRestoredFromDisk = true;
                RestoreSettingsLockFromDisk();
            }
            ...
            {
                // Everything here is now evaluated in Eastern time — the lock time
                // setting and the 5PM ET reset time share the same clock, so there's
                // no cross-timezone mismatch. The lock window is [most recent Eastern
                // lock-time occurrence, the next 5PM ET reset that follows it).
                var nowUtc = DateTime.UtcNow;
                var lastLockUtc = ComputeEasternOccurrenceUtc(_flattenLockTime.Value, nowUtc, findNext: false);
                var resetAfterLockUtc = ComputeNextResetUtc(lastLockUtc);

                bool inLockWindow = nowUtc >= lastLockUtc && nowUtc < resetAfterLockUtc;
        ...
        /// <summary>
        /// Converts an Eastern-time wall-clock time-of-day (e.g. the flatten/lock time
        /// setting, now interpreted as Eastern to match the 5PM ET reset time) into a
        /// concrete UTC instant, either the next upcoming occurrence or the most recent
        /// past occurrence relative to <paramref name="fromUtc"/>. Doing this conversion
        /// dynamically (rather than diffing raw TimeSpans) keeps it correct across DST.
        /// </summary>
        private DateTime ComputeEasternOccurrenceUtc(TimeSpan easternTimeOfDay, DateTime fromUtc, bool findNext)
        {
            var fromEastern = TimeZoneInfo.ConvertTimeFromUtc(fromUtc, _easternTz);
            var todayEastern = fromEastern.Date + easternTimeOfDay;
            DateTime occurrenceEastern;
            if (findNext)
                occurrenceEastern = fromEastern <= todayEastern ? todayEastern : todayEastern.AddDays(1);
            else
                occurrenceEastern = fromEastern >= todayEastern ? todayEastern : todayEastern.AddDays(-1);
            return TimeZoneInfo.ConvertTimeToUtc(
                DateTime.SpecifyKind(occurrenceEastern, DateTimeKind.Unspecified), _easternTz);
        }
        ...
        /// <summary>
        /// Ticks every second so the live "Trade Clock" row keeps counting up while a
        /// position is open. Only forces a redraw when there's actually something open;
        /// otherwise the row is static (showing the reset time) and doesn't need it.
        /// </summary>
        private void StartTradeClockTimer()
        {
            _tradeClockTimer?.Dispose();
            _tradeClockTimer = new Timer(_ =>
            {
                try
                {
                    if (HasOpenPositions)
                    {
                        // If we're already in a trade when the plugin loads (e.g. after a
                        // restart mid-trade), seed the clock from the earliest open position's
                        // OpenTime so the elapsed display is accurate, not reset to 0.
                        if (_tradeClockStartLocal == null)
                        {
                            var earliest = Core.Instance.Positions
                                .Where(p => _account != null && p.Account.Equals(_account) && p.OpenTime != default)
                                .OrderBy(p => p.OpenTime)
                                .FirstOrDefault();

                            if (earliest != null)
                            {
                                var openTime = earliest.OpenTime;
                                _tradeClockStartLocal = (openTime.Kind != DateTimeKind.Unspecified)
                                    ? openTime.ToLocalTime()
                                    : openTime; // assume local if unspecified
                            }
                            else
                            {
                                _tradeClockStartLocal = DateTime.Now;
                            }
                        }
                        _renderer?.RedrawBufferedGraphic();
                    }
                    ...
                    {
                        // Flat again — reset so the next trade starts back at 0.
                        _tradeClockStartLocal = null;
                    }
                }
                catch (Exception ex)
                {
                    Core.Instance.Loggers.Log($"[RiskManager] Trade clock timer error: {ex.Message}");
                }
        ...
        public override void Dispose()
        {
            ...
            _tradeClockTimer?.Dispose(); _tradeClockTimer = null;
        ...
        public override IList<SettingItem> Settings
        {
            get
            {
                ...
                { Text = "Session Lock Time 24hr Eastern (HH:MM, blank = off)", SortIndex = 9 });
            ...
            {
                base.Settings = value;
                bool isInitialLoad = !_initialSettingsApplied;
                if (_settingsLocked && !isInitialLoad) return;
                ...
                _renderer?.RedrawBufferedGraphic();
                if (isInitialLoad) { _initialSettingsApplied = true; ApplyPendingSettings(); }
            }
        }
        ...
        private void ScheduleFlattenLockTimer()
        {
            ...
            var nowUtc = DateTime.UtcNow;
            var nextLockUtc = ComputeEasternOccurrenceUtc(_flattenLockTime.Value, nowUtc, findNext: true);
            TimeSpan timeUntilLock = nextLockUtc - nowUtc;
            Core.Instance.Loggers.Log($"[RiskParameters] Next session lock in {timeUntilLock:hh\\:mm\\:ss} (Eastern time)");
            // Note: this timer re-fires every 24h on a fixed period. If a DST transition
            // falls between now and the next firing, the fire time can drift by an hour
            // until settings are re-saved (which reschedules against the current instant).
        ...
        private object LockSettings(object args = null)
        {
            Application.Instance.Notifications.ShowMessageBox(new MessageBoxParameters
            {
                ...
                Callback = result =>
                {
                    ...
                    PersistSettingsLock(ComputeNextResetUtc());
        ...
        // --- Settings-lock persistence (survives platform/plugin restarts, same as account lock) ---

        private DateTime ComputeNextResetUtc(DateTime? fromUtc = null)
        {
            var referenceUtc = fromUtc ?? DateTime.UtcNow;
            var referenceEastern = TimeZoneInfo.ConvertTimeFromUtc(referenceUtc, _easternTz);
            var todayResetEastern = referenceEastern.Date + _resetTimeEastern;
            var nextResetEastern = referenceEastern < todayResetEastern
                ? todayResetEastern
                : todayResetEastern.AddDays(1);
            return TimeZoneInfo.ConvertTimeToUtc(
                DateTime.SpecifyKind(nextResetEastern, DateTimeKind.Unspecified), _easternTz);
        }

        private string GetSettingsLockFilePath()
        {
            string key = _account?.Id ?? "default";
            foreach (char c in Path.GetInvalidFileNameChars())
                key = key.Replace(c, '_');
            return Path.Combine(_lockStateDir, $"settings_lock_{key}.state");
        }

        private void PersistSettingsLock(DateTime unlockAtUtc)
        {
            try
            {
                Directory.CreateDirectory(_lockStateDir);
                File.WriteAllText(GetSettingsLockFilePath(), unlockAtUtc.Ticks.ToString());
            }
            catch (Exception ex)
            {
                Core.Instance.Loggers.Log($"[RiskParameters] Failed to persist settings lock: {ex.Message}");
            }
        }

        private void ClearPersistedSettingsLock()
        {
            try
            {
                var path = GetSettingsLockFilePath();
                if (File.Exists(path)) File.Delete(path);
            }
            catch (Exception ex)
            {
                Core.Instance.Loggers.Log($"[RiskParameters] Failed to clear persisted settings lock: {ex.Message}");
            }
        }

        private void RestoreSettingsLockFromDisk()
        {
            try
            {
                var path = GetSettingsLockFilePath();
                ...
                var unlockAtUtc = new DateTime(ticks, DateTimeKind.Utc);
                if (DateTime.UtcNow < unlockAtUtc)
                {
                    _settingsLocked = true;
                    Core.Instance.Loggers.Log($"[RiskParameters] Restored settings lock from prior session — stays locked until {unlockAtUtc:u}.");
                }
                else
                {
                    // Reset time already passed while we were closed — clear the stale lock.
                    File.Delete(path);
                }
            }
            catch (Exception ex)
            {
                Core.Instance.Loggers.Log($"[RiskParameters] Failed to restore settings lock: {ex.Message}");
            }
        }
        ...
        /// <summary>True when the account has any open position (any symbol).</summary>
        public bool HasOpenPositions => _account != null &&
            Core.Instance.Positions.Any(p => p.Account.Equals(_account));
        public bool TradeCooldownEnabled => _tradeCooldownEnabled;
        public int TradeCooldownSeconds => _tradeCooldownSeconds;

        /// <summary>
        /// Elapsed time since a position was first detected open on the monitored account —
        /// a plain stopwatch we start/reset ourselves (see StartTradeClockTimer), not a read
        /// of any position's OpenTime field. Starts at 0 and counts up second by second while
        /// any position is open; null when flat.
        /// </summary>
        public TimeSpan? TradeClockElapsed
        {
            get
            {
                if (!HasOpenPositions || !_tradeClockStartLocal.HasValue) return null;
                var elapsed = DateTime.Now - _tradeClockStartLocal.Value;
    ...
    public class RiskManagerRenderer : Renderer
    {
        ...
        /// <summary>Formats a TimeSpan as mm:ss, switching to h:mm:ss once it reaches an hour.</summary>
        private static string FormatTradeClock(TimeSpan ts)
        {
        ...
        protected virtual void Draw(Graphics gr)
        {
            ...
            int lx = 10; int lv = lx + 88; int rx = mid + 2; int rv = rx + 78;
            ...
            // Open Contracts value starts closer to label so Mini+Micro counts have room.
            int rvOC = rx + 76;

            if (_plugin.CooldownActive)
            {
                // Trade Cooldown lockout: live seconds countdown, yellow.
                gr.DrawString("Cooldown", labelFont, grayBrush, rx, y);
                var cd = _plugin.CooldownRemaining;
                int cdTotalSeconds = (int)Math.Ceiling(cd.TotalSeconds);
                string cooldownStr = $"{cdTotalSeconds / 60}:{cdTotalSeconds % 60:D2}";
                gr.DrawString(cooldownStr, valueFont, yellowBrush, rv, y);
            }
            else if (_plugin.IsTriggered)
            {
                // Locked for any other reason (loss limit, profit target, schedule, max trades):
                // countdown to Session Reset Time.
                gr.DrawString("Cooldown", labelFont, grayBrush, rx, y);
                var lr = _plugin.LockRemaining;
                string lockStr;
                if (lr.TotalMinutes >= 60)
                {
                    int lrTotalMinutes = (int)Math.Ceiling(lr.TotalMinutes);
                    lockStr = $"{lrTotalMinutes / 60}:{lrTotalMinutes % 60:D2}";
                }
                else if (lr.TotalMinutes >= 1.0)
                {
                    lockStr = $"{(int)Math.Ceiling(lr.TotalMinutes)}";
                }
                else
                {
                    int totalSecs = (int)Math.Ceiling(lr.TotalSeconds);
                    lockStr = $"0:{totalSecs:D2}";
                }
                gr.DrawString(lockStr, valueFont, whiteBrush, rv, y);
            }
            else if (_plugin.TradeCooldownEnabled && _plugin.HasOpenPositions)
            {
                // Trade is open and cooldown is enabled: switch label to Open Contracts.
                gr.DrawString("Open Contracts", labelFont, grayBrush, rx, y);
                int openMini = _plugin.OpenMiniContracts;
                int openMicro = _plugin.OpenMicroContracts;
                ...
                string openMiniStr = $"{openMini} Mini  ";
                string openMicroStr = $"{openMicro} Micro";
                float miniW = gr.MeasureString(openMiniStr, valueFont).Width;
                gr.DrawString(openMiniStr, valueFont, miniOver ? redBrush : whiteBrush, rvOC, y);
                gr.DrawString(openMicroStr, valueFont, microOver ? redBrush : whiteBrush, rvOC + miniW, y);
            }
            else if (_plugin.TradeCooldownEnabled)
            {
                // Cooldown enabled, no open trade, no active cooldown lock:
                // display the configured cooldown duration so the user knows what to expect.
                gr.DrawString("Cooldown", labelFont, grayBrush, rx, y);
                gr.DrawString($"{_plugin.TradeCooldownSeconds} seconds", valueFont, grayBrush, rv, y);
            }
            else
            {
                gr.DrawString("Open Contracts", labelFont, grayBrush, rx, y);
            ...
            if (_plugin.FlattenLockTime.HasValue)
                gr.DrawString(DateTime.Today.Add(_plugin.FlattenLockTime.Value).ToString("h:mm tt ET"), valueFont, whiteBrush, rv, y);
            else gr.DrawString("OFF", valueFont, grayBrush, rv, y);
            y += lineH;

            // --- Row 5: Unrealized P&L | Reset Time / Trade Clock ---
            ...
            var tradeClockElapsed = _plugin.TradeClockElapsed;
            if (tradeClockElapsed.HasValue)
            {
                // In a trade — show a live-ticking clock timed off the earliest open position.
                gr.DrawString("Trade Clock", labelFont, grayBrush, rx, y);
                gr.DrawString(FormatTradeClock(tradeClockElapsed.Value), valueFont, whiteBrush, rv, y);
            }
            else
            {
                // Flat — show the fixed daily reset time.
