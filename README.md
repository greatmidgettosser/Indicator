RiskParameters.dll and Risk_Manager_v2.cs are the version without the ability to change the daily reset time! These are hardcoded to reset at 5:30 pm eastern time during the 1 hour that the futures market is closed. The Risk Manager version allows user to manually set the daily reset timer so if you lock the account, you can set the timer to be 1 minute from now and when that time hits the account will unlock.
