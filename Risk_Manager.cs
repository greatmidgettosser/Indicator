    ...
    public class RiskManagerPlugin : Plugin
    {
        ...
        public override IList<SettingItem> Settings
        {
            get
            {
                ...
                result.Add(new SettingItemString("FlattenLockTime", _flattenLockTime.HasValue ? _flattenLockTime.Value.ToString(@"hh\:mm") : "")
                ...
                { Text = "Session Reset Time 24hr local (HH:MM)", SortIndex = 4 });
                result.Add(new SettingItemBoolean("IncludeUnrealized", _includeUnrealized)
                { Text = "Include Unrealized P&L in Limits", SortIndex = 5 });
                result.Add(new SettingItemBoolean("CalculateFees", _calculateFees)
                { Text = "Calculate Commissions & Fees", SortIndex = 6 });
                ...
                foreach (var item in value)
                {
                    switch (item.Name)
                    {
                        ...
                        case "FlattenLockTime":
                            ...
                            {
                                _flattenLockTime = null;
                                _flattenLockTimer?.Dispose();
                                _flattenLockTimer = null;
                                Core.Instance.Loggers.Log("[RiskManager] Session Lock Time disabled.");
                            }
                            else if (TimeSpan.TryParse(flattenRaw, out var parsedFlatten))
                            {
                                _flattenLockTime = parsedFlatten;
                                ScheduleFlattenLockTimer();
                            }
                            break;
