...
namespace TradeJournal
{
    ...
    public class TradeJournalPlugin : Plugin
    {
        ...
        public override void Initialize()
        {
            ...
            this.Window.Browser.Layout.Row = 1;
        ...
        private void ApplyGridLayout()
        {
            this.Window.ReinitializeGridStructure(new NativeGridDefinition
            {
                ...
                {
                    // Row 0: trade list — fills most of the space
                    new NativeGridItemDefinitionDefinition(false, 1) { SizeType = NativeGridItemDefinitionSizeType.Star },
                    // Row 1: browser panel (trade note on top, main note on bottom)
                    new NativeGridItemDefinitionDefinition(false, 1) { SizeType = NativeGridItemDefinitionSizeType.Star }
                }
            });

            // Calendar spans both rows on the left.
            if (_calRenderer != null)
            {
                _calRenderer.Layout.Column = 0;
                _calRenderer.Layout.Row = 0;
                _calRenderer.Layout.RowSpan = 2;
            }

            // Trade list: right column, top row.
            if (_tlRenderer != null)
            {
            ...
            // Browser (trade note + main note): right column, bottom row.
        ...
        private void SaveTradeNoteFromBrowser(string tradeKey)
        {
            ...
            try
            {
                ...
                if (response?.Result is string raw)
                {
                    ...
                    // Refresh note-dot indicators in the trade list immediately after save
                    RenderTradeList();
                }
            }
            catch (Exception ex)
        ...
        private void OnTradeRowClicked(string tradeKey)
        {
            try
            {
                ...
                // Update the trade note header label
                string label = FormatTradeKeyLabel(tradeKey);
                this.Window.Browser.UpdateHtml("tradeNoteLabel", HtmlAction.SetTextContent, label);
        ...
        private static string FormatTradeKeyLabel(string tradeKey)
        {
            var parts = tradeKey.Split('|');
            if (parts.Length != 2) return tradeKey;
            string symbol = TradeJournalPlugin.GetSymbolRoot(parts[0]);
        ...
        private void ClearTradeNoteSelection()
        {
            ...
            this.Window.Browser.UpdateHtml("tradeNoteLabel", HtmlAction.SetTextContent, "select a trade above");
        ...
        private void RenderTradeList()
        {
            ...
            try
            {
                ...
                _tlRenderer.SetTrades(listResult, showDaySeparators, _selectedTradeKey, noteKeys);
            }
            catch (Exception ex)
            {
    ...
    public class TradeJournalCalendarRenderer : Renderer
    {
            ...
            Font fontHdr, Font fontNames, Font fontCount)
        {
            ...
            if (_showAllTradesView)
            {
                ...
                DrawAllRow("Trades", am.RoundTrips > 0 ? $"{am.RoundTrips} ({am.WinCount} W : {am.LossCount} L)" : am.RoundTrips.ToString(), grayBrush);
    ...
    public class TradeJournalTradeListRenderer : Renderer
    {
        ...
        private static readonly string[] ColHeaders =
            { "Date", "Entry Time", "Exit Time", "Symbol", "Qty", "Side", "Entry Price", "Exit Price", "Hold Time", "P&L" };
        ...
        public void SetTrades(TradeListResult result, bool showDaySeparators, string selectedTradeKey, HashSet<string> tradeNoteKeys)
        {
            ...
            _scrollOffsetY = 0; // reset scroll on data change
            _bufferedGraphic.IsDirty = true;
        }
        ...
        private void Draw(Graphics gr)
        {
            ...
            var fontRow = new Font("Consolas", 10.8f, FontStyle.Regular);
            ...
            // Date EntryT ExitT Symbol Qty Side EntryP ExitP Hold PnL
            int[] colW = {
                (int)(w * 0.09f),  // Date
