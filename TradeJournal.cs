...
namespace TradeJournal
{
    ...
    public class TradeJournalPlugin : Plugin
    {
        ...
        // ===================================================================
        // BUGFIX REGION — startup rendering + multi-instance safety
        // ===================================================================
        //
        // BUG 1 ("first load never renders the trades or the notes"):
        //   Everything downstream of the DOM-readiness check used to hang off the
        //   Window.Browser.BrowserLoaded event, which was subscribed in Populate().
        //   The very first time the panel is created manually, the browser has
        //   already finished loading layout.html by the time Populate runs, so the
        //   event never fires again — AttemptInitialLoad was never called, so
        //   _browserReady stayed false, InitializeTradeList() never ran, and
        //   _tradeListReady stayed false, which makes RenderTradeList() return
        //   immediately forever. The calendar renderer paints itself independently
        //   of all that, which is exactly why the calendar showed up but the trade
        //   list and the notes stayed blank until Quantower was fully restarted
        //   (on restart the panel is rebuilt from the workspace and the event
        //   happens to fire in time).
        //
        //   Fix: initialization is now driven by a self-retrying background pump
        //   started unconditionally from Populate(). BrowserLoaded is still handled,
        //   but only as an *additional* trigger (and to re-push content if the page
        //   ever reloads), never as the only one. All of it is idempotent.
        //
        // BUG 2 ("opening a second instance freezes the platform"):
        //   Three separate contributors, all fixed below.
        //   (a) GetHtmlValue is a blocking round-trip to the browser process. It was
        //       being called straight from the UI thread (mouse-click handlers ->
        //       OnDaySelected/OnPrevMonth/OnNextMonth -> SaveNoteFromBrowser, and
        //       BrowserLoaded -> AttemptInitialLoad). While a second CEF browser is
        //       being created, the UI thread has to keep pumping messages; blocking
        //       it there deadlocks the whole platform. Browser reads now happen off
        //       the UI thread with a hard timeout, so they can never hang Quantower.
        //   (b) A second instance duplicated every global side effect — most
        //       damagingly two screenshot captures per fill, both full-screen BitBlts
        //       racing to write the same file path. Screenshot capture is now owned
        //       by a single designated primary instance.
        //   (c) Teardown leaked: BrowserLoaded was never unsubscribed, the screenshot
        //       strip renderer was never disposed, and the retry timers outlived the
        //       window, so they went on poking this.Window.Browser after it was gone.
        //       Dispose now tears everything down behind a _disposed guard.

        // Escape hatch. Multiple panels are supported and safe with the fixes below,
        // but if a second panel ever misbehaves again on your build, flip this to
        // false: additional panels then open as an inert placeholder that wires up
        // nothing at all, instead of duplicating timers, handlers and captures.
        ...
        public override void Dispose()
        {
            ...
            try { _ssRenderer?.Dispose(); } catch { }   // was leaked
            _calRenderer = null;
            _tlRenderer = null;
            _ssRenderer = null;
        ...
        protected override void OnLayoutUpdated()
        {
            ...
            // NOTE: only the calendar and the browser get NonClientMargin, exactly as
            // the original did. Applying it to the trade list and screenshot strip as
            // well was tried as a fix for the invisible caption buttons; it did not
            // make them visible and it pushed the whole panel down the window, so it
    ...
    public class TradeJournalCalendarRenderer : Renderer
    {
            ...
            Font fontHdr, Font fontNames, Font fontCount)
        {
            ...
            if (_showAllTradesView)
            {
                ...
                int rowYRight = rowY; // right column tracks independently so we can reset if needed
                ...
                var yellowBrush = new SolidBrush(Color.FromArgb(255, 220, 50));
                for (int si = 0; si < extra.TimeSlots.Length; si++)
                {
        ...
        private void DrawYearlyMetricsPanel(Graphics gr, int panelY,
            SolidBrush whiteBrush, SolidBrush grayBrush, SolidBrush lightGray,
            SolidBrush greenBrush, SolidBrush redBrush,
            Font fontHdr, Font fontNames, Font fontCount)
        {
        ...
        private void Draw(Graphics gr)
        {
            ...
            if (_showYearlyMetrics)
                DrawYearlyMetricsPanel(gr, panelY, whiteBrush, grayBrush, lightGray,
    ...
    public class TradeJournalTradeListRenderer : Renderer
    {
        ...
        private void Draw(Graphics gr)
        {
            ...
            var whiteBrush = new SolidBrush(Color.FromArgb(220, 220, 220));
            var grayBrush = new SolidBrush(Color.FromArgb(120, 120, 120));
            var dimBrush = new SolidBrush(Color.FromArgb(80, 80, 80));
            var greenBrush = new SolidBrush(Color.FromArgb(0, 200, 100));
            var redBrush = new SolidBrush(Color.FromArgb(224, 72, 60));
            var selBrush = new SolidBrush(Color.FromArgb(26, 107, 58));
            var hoverBrush = new SolidBrush(Color.FromArgb(42, 42, 42));
            var sepBgBrush = new SolidBrush(Color.FromArgb(32, 32, 32));
            var headerBgBrush = new SolidBrush(Color.FromArgb(30, 30, 30));
            var noteDotBrush = new SolidBrush(Color.FromArgb(0, 200, 100));
            ...
            gr.DrawLine(new Pen(Color.FromArgb(60, 60, 60)), 0, HeaderRowH - 1, bounds.Width, HeaderRowH - 1);
            ...
            else
            {
                ...
                foreach (var t in _trades)
                {
                    ...
                    if (visible)
                    {
                        ...
                        gr.DrawLine(new Pen(Color.FromArgb(38, 38, 38)), 0, rowScreenY + RowH - 1, bounds.Width, rowScreenY + RowH - 1);
    ...
    public class TradeJournalScreenshotStripRenderer : Renderer
    {
        ...
        private void Draw(Graphics gr)
        {
            ...
            using var numBrush = new SolidBrush(Color.FromArgb(0, 200, 100));
            using var boxBrush = new SolidBrush(Color.FromArgb(42, 42, 42));
