...
namespace TradeJournal
{
    ...
    public class TradeJournalPlugin : Plugin
    {
        ...
        private void AttemptInitialLoad(string date, int attemptsLeft)
        {
            try
            {
                ...
                // Read it back via the same JS query SaveNoteFromBrowser relies on.
                var response = this.Window.Browser.GetHtmlValue("noteArea", HtmlGetValueAction.GetValueByJsQuery,
        ...
        private void OnTradeAdded(Trade trade)
        {
            try
            {
                ...
                _weekMetricsCache.Remove(TradeJournalPlugin.GetWeekMonday(tradeDate).ToString("yyyy-MM-dd"));
        ...
        private void SaveNoteFromBrowser(string date)
        {
            ...
            try
            {
                // Read innerText from the contenteditable div via JS query.
                // innerText gives us the plain text the user sees — newlines included —
                // without any HTML tags that contenteditable might inject.
                var response = this.Window.Browser.GetHtmlValue(
                    "noteArea", HtmlGetValueAction.GetValueByJsQuery,
                    "document.getElementById('noteArea').innerText");

                if (response?.Result is string content)
                {
                    // Browsers append a trailing newline to innerText of contenteditable
                    // divs; trim it so files don't accumulate extra blank lines.
        ...
        private void LoadNote(string date)
        {
            try
            {
                ...
                // Escape content for safe insertion as HTML inner content.
                // Pre-wrap CSS handles newlines so we only need to escape < > &.
        ...
        // Per-week aggregate metrics keyed by the Monday of that week (yyyy-MM-dd)
        private Dictionary<string, WeekMetrics> _weekMetricsCache
            = new Dictionary<string, WeekMetrics>();
        ...
        public WeekMetrics GetWeekMetrics(string dateStr)
        {
            if (!DateTime.TryParse(dateStr, out DateTime date))
                return new WeekMetrics();

            DateTime weekMonday = GetWeekMonday(date);
            string cacheKey = weekMonday.ToString("yyyy-MM-dd");
    ...
    public class TradeJournalCalendarRenderer : Renderer
    {
            ...
            Font fontHdr, Font fontNames, Font fontCount)
        {
            ...
            var metrics = _plugin.GetWeekMetrics(dateStr);
            ...
            int pieSize = Math.Min(200, availableW);
            if (pieSize > 20)
                DrawWinLossPie(gr, panelX + 2 * colW + 4, panelY + 14, pieSize, metrics.Pie, fontCount, whiteBrush);
