...
namespace TradeJournal
{
    public class TradeJournalPlugin : Plugin
    {
        ...
        public override void Initialize()
        {
            ...
            // Set up grid: left column 240px fixed for calendar, right column fills rest for browser
        ...
        {
            Core.Instance.Loggers.Log($"[TradeJournal] OnDaySelected: {date} | current: {_selectedDate}");
            SaveNoteForDate(_selectedDate);
            _selectedDate = date;
        ...
        private void SaveNoteForDate(string date)
        {
            ...
            try
            {
                ...
                Core.Instance.Loggers.Log($"[TradeJournal] Save response: success={response?.Success} result={response?.Result} message={response?.Message}");
            ...
            {
                Core.Instance.Loggers.Log($"[TradeJournal] Save error: {ex.Message}");
            }
        }
        ...
        private void SaveCurrentNote()
        {
            ...
            try
            {
                ...
                if (response?.Result is string content)
                {
                    string path = Path.Combine(JournalFolder, $"{_selectedDate}.txt");
                    if (string.IsNullOrWhiteSpace(content))
                    {
                        if (File.Exists(path)) File.Delete(path);
                    }
    ...
    public class TradeJournalCalendarRenderer : Renderer
    {
        ...
        // Hit rects built during draw, used for click detection
        ...
        private void Draw(Graphics gr)
        {
            ...
            gr.DrawString("‹", new Font("Arial", 14, FontStyle.Bold), arrowBrush,
                _prevBtnRect, sf);
            gr.DrawString("›", new Font("Arial", 14, FontStyle.Bold), arrowBrush,
                _nextBtnRect, sf);
