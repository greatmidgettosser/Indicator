    ...
    public class TradeJournalPlugin : Plugin
    {
        ...
        private void RenderTradeList()
        {
            ...
            try
            {
                ...
                if (isMonthly)
                    listResult = GetTradesForMonth(_currentYear, _currentMonth + 1);
                else if (isWeekly && _calRenderer.WeeklyViewDate != null)
                    listResult = GetTradesForWeek(_calRenderer.WeeklyViewDate);
                else
                    listResult = GetTradesForDay(_selectedDate);
        ...
        public TradeListResult GetTradesForDay(string date)
        {
            var dm = GetDayMetrics(date, null);
        ...
        public TradeListResult GetTradesForWeek(string dateStr)
        {
            var days = new List<(string DayKey, List<RoundTripTrade> Trades)>();
            if (!DateTime.TryParse(dateStr, out DateTime date))
                return new TradeListResult { Days = days, TruncatedCount = 0 };
            ...
            for (int d = 0; d < 5; d++) // Mon=0 … Fri=4
            {
                DateTime day = weekMonday.AddDays(d);
                string dayKey = day.ToString("yyyy-MM-dd");
                var dm = GetDayMetrics(dayKey, null);
                if (dm.Trades.Count > 0)
        ...
        public TradeListResult GetTradesForMonth(int year, int month)
        {
            var days = new List<(string DayKey, List<RoundTripTrade> Trades)>();
            int daysInMonth = DateTime.DaysInMonth(year, month);
            for (int day = 1; day <= daysInMonth; day++)
